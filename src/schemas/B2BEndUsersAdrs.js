...
// --- Begin: B2B Partner End Users Addresses Schema --- //
const schema = new Schema({
  ...
  adrsType: {type: String, required: true}, // USA National ID, USA Residential, Office
  adrsName: {type: String, required: false}, // Office: MetrixIT, Teksolve IT
  hNum: {type: String, required: true}, // Building Name, House Number, Floor
  area: {type: String, required: false}, // Streat, Area, Village
  aLocality: {type: String, required: false}, // Area Locality, Mandal
  zip: {type: String, required: true}, // Zip Code, Pincode
...
schema.index({city: 'text', state: 'text', zip: 'text', hNum: 'text', area: 'text'});
schema.index({delFlag: -1, b2b: 1});
schema.index({delFlag: -1, b2b: 1, uid: 1});
schema.index({delFlag: -1, b2b: 1, uid: 1, adrsType: 1});
schema.index({cDtStr: -1, uDtStr: -1});

module.exports = mongoose.model(config.collB2BEndUsersAdrs, schema);
// --- End: B2B Partner End Users Addresses Schema --- //
