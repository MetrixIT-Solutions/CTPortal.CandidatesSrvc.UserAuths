...
// --- Begin: B2B Partner End Users Leads Closed Schema --- //
const schema = new Schema({
  ...
  leadId: {type: String, required: true, index: true, unique: true},
  ...
  uuName: {type: String, required: true}, // Updated Users.uuName
  uDate: {type: Date, required: true}, // Date & Time
  uDtStr: {type: String, required: true}, // Date & Time String - Format = YYYY-MM-DD HH:mm:ss
}, {collection: config.collB2BEuLeadsClsd});

schema.index({searchStr: 'text'});
schema.index({mSearchStr: 'text'});
schema.index({delFlag: -1, b2b: 1, org: 1, wStatus: 1});
schema.index({delFlag: -1, b2b: 1, org: 1, euUser: 1});
schema.index({delFlag: -1, b2b: 1, org: 1, pReports: 1, wStatus: 1});
schema.index({delFlag: -1, b2b: 1, org: 1, wStatus: 1, team: 1});
schema.index({cDtStr: -1, uDtStr: -1});

module.exports = mongoose.model(config.collB2BEuLeadsClsd, schema);
// --- End: B2B Partner End Users Leads Closed Schema --- //
